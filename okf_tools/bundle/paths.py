...
def resolve_bundle_path(bundle_root: Path, source_path: Path, target: str) -> Path | None:
    """Resolve an OKF bundle path without allowing it to leave the bundle."""
    target_path = PurePosixPath(target)
    if target_path.is_absolute():
        candidate = bundle_root.joinpath(*target_path.parts[1:])
    else:
        candidate = source_path.parent.joinpath(*target_path.parts)
    try:
